...
/*******************************************************************************
File Name:  tcp_performance_test.c
...
void TCPTXPerformanceTask(void)
{
	...
	while(w >= 10+29+12+32u)
	{
		...
		TCPIP_TCP_ArrayPut(MySocket, vBuffer, sizeof(vBuffer));                                        // 10 B
		TCPIP_TCP_StringPut(MySocket, (const uint8_t*)": We are currently achieving ");                // 29 B
		TCPIP_TCP_ArrayPut(MySocket, (const uint8_t*)"       ", 12-strlen((char*)vBytesPerSecond));     
		TCPIP_TCP_StringPut(MySocket, vBytesPerSecond);                                                // 12 B 
		TCPIP_TCP_StringPut(MySocket, (const uint8_t*)"00 bytes/second TX throughput.\r\n");           // 32 B
...
void TCPRXPerformanceTask(void)
{
	...
	if(dw > SYS_TICK_TicksPerSecondGet())
	{
		...
		TCPIP_TCP_StringPut(MySocket, (const uint8_t*)" bytes/second\r\n");
