...
#include "main_demo.h"
   ...
   || defined(TCPIP_STACK_USE_ICMP_SERVER) \
   || defined(TCPIP_STACK_USE_IPV6))     \
   && defined(APP_USE_PING_DEMO)
...
bool Ping4 (char * target)
{
    ...
    pNetIf = TCPIP_STACK_NetDefaultGet();
...
void PingDemoTask (void)
{
    switch (pingState)
    {
        ...
        case STATE_DNS_SEND_QUERY_IPV4:
            if (!TCPIP_DNS_UsageBegin(pNetIf))
                return;

            if (TCPIP_DNS_Resolve((const char *)targetHostName, DNS_TYPE_A) != DNS_RES_OK)
                return;
        ...
        case STATE_DNS_GET_RESPONSE_IPV4:
            {
                ...
                if ((long)(SYS_TICK_Get() - pingTimer) > 0)
                {
                    ...
                    TCPIP_DNS_UsageEnd(pNetIf);
                    pingState = STATE_IDLE;
                    return;
                } 
    
                res = TCPIP_DNS_IsResolved((const char *)targetHostName, &targetAddressIPv4);
		...
		case STATE_RESOLVE_ARP:
            if ((targetAddressIPv4.Val & TCPIP_STACK_NetMask(pNetIf)) == TCPIP_STACK_NetMask(pNetIf))
  			...
  			TCPIP_ARP_Resolve(pNetIf, &firstHopAddress);
			pingTimer = SYS_TICK_Get();
			pingState = STATE_ARP_RESOLVED;
			break;

		case STATE_ARP_RESOLVED:
			if(!TCPIP_ARP_IsResolved(pNetIf, &firstHopAddress, &targetMACAddr))
			{
        ...
        case STATE_SEND_ECHO_REQUEST_IPV4:
            {
                ...
                memcpy (&info.MACAddr, &targetMACAddr, sizeof (TCPIP_MAC_ADDR));
                TCPIP_ICMP_EchoRequestSend (&info, ++wICMPSequenceNumber, 0xBEEF);
