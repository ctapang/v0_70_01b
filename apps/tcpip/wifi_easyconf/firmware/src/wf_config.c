...
void DRV_WIFI_ProcessEvent(uint16_t event, uint16_t eventInfo)
{
    ...
    switch (event)
    {
        ...
        /*--------------------------------------*/  
            SYS_CONSOLE_MESSAGE("MRF24W Event: Scan Results Ready,");
        ...
        /*--------------------------------------*/
            SYS_CONSOLE_PRINT("MRF24W Event: WiFi error occurred (%d) See DRV_WIFI_MGMT_ERRORS and DRV_WIFI_MGMT_ERRORS", eventInfo);
            break;

        default:
            SYS_ASSERT(false, "MRF24W Event: Unknown WiFi Event");  /* unknown event */
            break;
    }        
