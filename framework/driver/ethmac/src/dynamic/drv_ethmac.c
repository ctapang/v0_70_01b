...

{
	...
	while(1)
	{
		...
		pauseType=(initData->ethFlags & ETH_OPEN_FDUPLEX)?ETH_MAC_PAUSE_CPBL_MASK:ETH_MAC_PAUSE_TYPE_NONE;
        ...
        phyInitRes = DRV_ETHPHY_Setup(hPhyClient, initData->ethFlags, initData->phyFlags, &linkFlags);
        ...
        if(initData->ethFlags & ETH_OPEN_AUTO)
        ...
        pMacD->mData._linkWaitTick = ((SYS_TICK_ResolutionGet() * initData->linkInitDelay) + 999 ) / 1000;
