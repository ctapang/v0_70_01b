...
/*Wi-Fi Driver Version Number*/
#define DRV_WIFI_VERSION_NUMBER                 "3.1.0"
...
/*************************************************************************
  ...
  Description:
    ...
    This enumeration identifies the default Wi-Fi scan context values that
...
#ifdef MRF24W_EASYCONFIG_DEMO
#define DRV_WIFI_DEFAULT_ADHOC_PRESCAN                DRV_WIFI_ENABLED            /* Conduct scan prior to connecting to network */
#else
#define DRV_WIFI_DEFAULT_ADHOC_PRESCAN                DRV_WIFI_DISABLED            /* Conduct scan prior to connecting to network */
#endif
...
/***********************************************************
  ...
  Description:
    ...
    This enumeration identifies Wi-Fi Power save states. See
...
/******************************************************************************
  ...
  Description:
    ...
    DRV_WIFI_SECURITY_WPA_AUTO_WITH_KEY           Auto-select between WPA/WPA2
                                                   with binary key
    DRV_WIFI_SECURITY_WPA_AUTO_WITH_PASS_PHRASE   Auto-select between WPA/WPA2
                                                   with passphrase
  
  WPA_AUTO_WITH_PASS_PHRASE   Auto-select between WPA/WPA2
                                                   with passphrase
...
typedef struct 
{
    ...
    uint8_t wpsPinLength;                       
        ...
        /**********************************************************************
          pointer to where the Wi-Fi driver will store passphrase info (must be
          global memory)                                                       
          **********************************************************************/
...
/*******************************************************************************
  ...
  Description:
    ...
    Preamble   0: AP transmitting with short preamble 1: AP transmitting with
                \long preamble
    WPA        Only valid if Privacy is 1. 0: AP does not support WPA 1: AP
                supports WPA
    WPA2       Only valid if Privacy is 1. 0: AP does not support WPA2 1: AP
                supports WPA2
    </table>
...
/*************************************************************************
  ...
  Remarks:
    ...
    future. The Wi-Fiinitialization takes place in a state machine called
    by MRF24W_MACInit().                                                  
  *************************************************************************/
...
/***************************************************************************
  ...
  Conditions:
    Wi-Fi initialization must be complete.
  Input:
    p_networkType -              Pointer to where the network type will be
                                 written.
...
/***********************************************************************
  ...
  Summary:
    Sets the BSSID set in DRV_WIFI_BssidSet().
  Description:
...
/*****************************************************************************************
  ...
  Description:
    ...
    of two ways: 1) Beacon timeout 2) Deauth received from AP
  ...
  Input:
    retryCount -           Number of times the MRF24WG should try to regain
                           the connection (see description)<p />
    deauthAction -         In the event of a deauth from the AP, the action
                           the MRF24WG should take (see description)<p />
    beaconTimeout -        Number of missed beacons before the MRF24WG
                           designates the connection as lost (see
                           \description)<p />
...
/************************************************************************************************
  ...
  Conditions:
    Wi-Fi initialization must be complete.
  Input:
    p_retryCount -           Pointer where retry count is written<p />
    p_deauthAction -         Pointer where deauth action is written<p />
    p_beaconTimeout -        Pointer where beacon timeout is written<p />
...
/***********************************************************************
  ...
  Conditions:
    Wi-Fi initialization must be complete and relevant connection
    \parameters must have been set.
  Return:
    None
  Example:
    <code>
...
/**************************************************************************
  ...
  Input:
    ...
    1\-255 -            number of consecutive TX failures before connection
...
/**************************************************************************
  ...
  Description:
    During WPS the MRF24WG will receive a passphrase. That passphrase must
    be converted to a binary key. The MRF24WG can do this, but it will take
    20-30 seconds. The host MCU will be able to convert the passphrase to a
    binary key much faster. If that is desired, then this function should
    be called to request the MRF24WG to signal the
    DRV_WIFI_EVENT_KEY_CALCULATION_REQUEST event when the passphrase is
    ready. Then the host can retrieve the passphrase (see
    DRV_WIFI_WPSCredentialsGet()), convert to a binary key (see
    DRV_WIFI_ConvPassphraseToKey()), and send it back to the MRF24WG via
    the DRV_WIFI_SetPSK() function.
  Conditions:
    Wi-Fi initialization must be complete.
  Return:
    None.
...
/*****************************************************************************
  ...
  Conditions:
    Wi-Fi initialization must be complete.
  Input:
    p_powerSaveState -  Pointer to where power state is written (see
                        \description)
  Return:
    None.
  Example:
...
/****************************************************************************************************
  ...
  Description:
    ...
    filterId -- DRV_WIFI_MULTICAST_FILTER_1 thru
...
/***************************************************************************
  ...
  Input:
    ...
    If true -  * All scan results are retained (both Infrastructure and Ad
                 Hoc networks).
