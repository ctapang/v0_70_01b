...
/*************************************************************************************
  ...
  Conditions:
    None.
  Input:
    drvIndex -  Index for the driver instance to be initialized<p />
    init -      Pointer to a data structure containing any data necessary to
...
/**************************************************************************
  ...
  Description:
    ...
    DRV_IO_INTENT_BLOCKING, these functions will block till the required
  ...
  Conditions:
    ...
    this function.
  Input:
    drvIndex -  Identifier for the object instance to be opened<p />
...
/**************************************************************************************
  ...
  Input:
    handle -    A valid open\-instance handle, returned from the driver's
                open routine<p />
    rxBuffer -  The buffer to which the data should be written to.<p />
  ...
  Remarks:
    If the DRV_IO_INTENT_BLOCKING flag was given in the call to the
    DRV_SPI_Open function and the driver was built to support blocking
    behavior the call will block until the operation is complete.
    
    If the DRV_IO_INTENT_NONBLOCKING flag was given in the call to the
    DRV_SPI_Open function or the driver was built to only support
    non-blocking behavior the call will return immediately. If the driver
    was not able to accept the data, an overrun error status will be
    captured. To ensure that the driver is ready to accept data, the caller
...
/***************************************************************************************
  ...
  Input:
    handle -    A valid open\-instance handle, returned from the driver's
                open routine<p />
    txBuffer -  The buffer which hold the data.<p />
    size -      Number of bytes to be read.
  Return:
    None.
  Example:
    ...
    // PUT API returns data in any case, it is up to the user to use it
    DRV_SPI_BufferAddWrite( handle, myBuffer, 10 );
...
// *****************************************************************************
/* Function:
    size_t DRV_SPI_BufferProcessedSizeGet ( DRV_SPI_BUFFER_HANDLE bufferHandle );
  ...
  Precondition:
    ...
    Either the DRV_SPI_BufferAddRead or DRV_SPI_BufferAddWrite function 
  ...
  Example:
    ...
    void APP_SPIBufferEventHandler( DRV_SPI_BUFFER_EVENT event, 
            DRV_SPI_BUFFER_HANDLE bufferHandle, uintptr_t contextHandle )
    {
        // The context handle was set to an application specific
        // object. It is now retrievable easily in the event handler.
        ...
        switch(event)
        {
            ...
            case DRV_SPI_BUFFER_EVENT_FAILURE:
                ...
                processedBytes = DRV_SPI_BufferProcessedSizeGet(bufferHandle);
