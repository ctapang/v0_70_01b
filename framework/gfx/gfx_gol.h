...
/**************************************************************************
  ...
  Description:
    This callback function is implemented by the application. This is
    called inside the GFX_GOL_ObjectListDraw() function when the drawing of
    objects in the active list is completed.
    
    Any application specific rendering must be performed on this callback
    function so no object rendering will be affected by the application
    calls to primitive rendering functions. Application setting the drawing
    color, line style, fill style, text string cursor position and current
    font will not affect the object rendering. This is also the safe place
    to modify the active list.
    
    When the application has performed its own primitive rendering calls,
    this function must return true to inform the GFX_GOL_ObjectListDraw()
    that it is done rendering and checking for object drawing or redrawing
    can continue.
  Conditions:
    None.
  Return:
    true - is returned when application rendering is done. false - is
    returned when application rendering is not yet finished.
  Example:
    None.                                                                  
  **************************************************************************/
typedef bool (*GFX_GOL_DRAW_CALLBACK_FUNC) (void);
...
/**************************************************************************
  ...
  Summary:
    Message callback function definition. This application defined function
    allows the application to perform application specific processing of
    user messages.
  Description:
    This application defined function is called by the
    GFX_GOL_ObjectMessage() function allowing the application the
    opportunity to process the user messages and customize object behavior
    as well as application controlled functions.
    
    GFX_GOL_ObjectMessage() calls this function when a valid message for an
    object in the active list is received. Application implements any
    action for the message in this callback function. If this callback
    \function returns true, the message for the object will be processed
    using the default action of the object. If false is returned, the
    default action will not be performed. In this case, it is assumed that
    this callback function has performed the appropriate changes to the
  ...
  Return:
    true - When true is returned, the object will set its state depending
    on the translated messages. false - When false is returned, the object
    will not process the translated message and will assume the application
    has performed necessary action on the message.
  Example:
    None.                                                                  
  **************************************************************************/
typedef bool (*GFX_GOL_MESSAGE_CALLBACK_FUNC)       \
...
/**************************************************************************
  ...
  Summary:
    This function sets the draw callback function that the application will
    use to render application specific rendering.
  Description:
    This function sets the draw callback function that the application will
    use to call primitive function to implement application specific
    shapes. See GFX_GOL_DRAW_CALLBACK_FUNC definition for details on the
...
/**************************************************************************
  ...
  Summary:
    ...
    objects, object states are modified based on the default behavior or
    user-defined behavior.
  Description:
    This function receives a GFX_GOL_MESSAGE message from user and loops
    through the active list of objects to check which object is affected by
    the message. For affected objects the message is translated and
    GFX_GOL_ObjectMessageCallback() is called. In the call back function,
    user has the ability to implement action for the message. If the call
    back function returns non-zero, OBJMsgDefault() is called to process
    message for the object by default. If zero is returned OBJMsgDefault()
    is not called. Please refer to GOL Messages section for details.
  Conditions:
    None.
  Input:
